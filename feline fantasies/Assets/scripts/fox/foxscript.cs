...
public class foxscript : MonoBehaviour
{
    ...
    void Update()
    {
        if (isPlayerNearby && !isTalking && Input.GetKeyDown(KeyCode.E))
        {
            if (!isQuestStarted)
            {
                StartDialogue(dialogueStartQuest, squirrelIcon, "Fox");
            }
            else if (isQuestStarted && !isQuestCompleted && player.foodsCollected < 4)
            {
                StartDialogue(dialogueDuringQuest, squirrelIcon, "Fox");
            }
            else if (isQuestStarted && player.foodsCollected >= 4)
            {
                ReturnNutsToSquirrel();
            }
            else if (isQuestCompleted)
            {
                StartDialogue(dialogueQuestComplete, squirrelIcon, "Fox");
            }
        }
    }
    ...
    public void ShowNextDialogue()
    {
        if (isTalking)
        {
            ...
            {
                ShowDialogueLine(activeDialogue, squirrelIcon, "Fox");
            }
            else
            {
                EndDialogue();
    ...
    private string[] GetActiveDialogue()
    {
        ...
        if (!isQuestCompleted && player.foodsCollected < 4)
    ...
    private IEnumerator WaitForClosingAnimation()
    {
        ...
        else if (player.foodsCollected >= 4 && !isQuestCompleted)
        {
            ReturnNutsToSquirrel();
    ...
    {
        Debug.Log("Starting the fox quest!");
        questManager.StartFoxQuest();
        isQuestStarted = true;
    ...
    {
        if (player.foodsCollected >= 4)
        {
            Debug.Log("giving food to fox!");
            player.foodsCollected = 0;
            isQuestCompleted = true;
            questManager.CompleteFoxQuest();
            nutCollectorPanel.SetActive(false);
            StartDialogue(dialogueQuestComplete, squirrelIcon, "Fox");
    ...
    private void ActivateEncyclopedia()
    {
        ...
        StartCoroutine(FadeInMessage("QUEST COMPLETE: Fox has been added to Encyclopedia"));
    }
